...
# 在不考虑潜在异常值的情况下重复该分析
full.model_no_outliers <- rma.mv(yi , vi, slab = author, random = ~ 1 | studyid, test = "t", method = "REML", data = my_data)
...
Per_session_exercise_duration.model2 <- rma.mv(yi, vi, slab = author, mods = ~ factor(Per_session_exercise_duration_coded), random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Per_session_exercise_duration.model2

Per_session_exercise_duration.model<- rma.mv(yi, vi, slab = author, mods = ~ factor(Per_session_exercise_duration_coded)- 1, random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Per_session_exercise_duration.model

Frequency.model2 <- rma.mv(yi,vi, slab = author, mods = ~ factor(Frequency_coded), random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Frequency.model2

Frequency.model <- rma.mv(yi,vi, slab = author, mods = ~ factor(Frequency_coded) - 1, random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Frequency.model

Exercise_time_per_week.model2<- rma.mv(yi, vi, slab = author, mods = ~ factor(Exercise_time_per_week_coded), random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Exercise_time_per_week.model2

Exercise_time_per_week.model<- rma.mv(yi, vi, slab = author, mods = ~ factor(Exercise_time_per_week_coded) - 1, random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
Exercise_time_per_week.model

int_weeklyvolume.model2<- rma.mv(yi, vi, slab = author, mods = ~ factor(int_weeklyvolume_coded), random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
int_weeklyvolume.model2

int_weeklyvolume.model<- rma.mv(yi,vi, slab = author, mods = ~ factor(int_weeklyvolume_coded) - 1, random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data)
int_weeklyvolume.model
...
res.lin <- rma(yi, vi, mods = ~ int_weeklyvolume, data=res )
res.lin
regplot(res.lin, las=1, digits=1, bty="o", 
        xlab="total intervention duration(Wks)", ylab="Hedges'g")

res.cub <- rma(yi,vi,, mods = ~ poly(int_weeklyvolume, degree=3, raw=TRUE), data=res )
res.cub
...
res.rcs3<- rma(yi,vi,mods = ~ rcs(int_weeklyvolume, 3), data=res)
res.rcs3
...
#线性回归(Frequency)
res.lin <- rma(yi, vi, mods = ~ Frequency, data=res )
res.lin
regplot(res.lin, las=1, digits=1, bty="o", 
        xlab="Exercise frequency", ylab="Hedges'g")
...
res.lin
regplot(res.lin, las=1, digits=1, bty="o", 
        ylab="Hedges'g", xlab="single exercise duration(mins)")
...
res.lin <- rma(yi,vi, mods = ~ Exercise_time_per_week, data=res )
res.lin
regplot(res.lin, las=1, digits=1, bty="o", 
        ylab="Hedges'g", xlab="weekly exercise duration(mins)")
res.cub <- rma(yi, vi, mods = ~ poly(Exercise_time_per_week, degree=3, raw=TRUE), data=res )
res.cub
...
res.rcs3<- rma(yi,vi,mods = ~ rcs(Exercise_time_per_week, 3), data=res)
res.rcs3
...
#三样条4节点
res.rcs4 <- rma(yi,vi,mods = ~ rcs(Exercise_time_per_week, 4), data=res)
res.rcs4
...
res.cub
xs <- seq(0, 10, length=5)
sav <- predict(res.cub, newmods=unname(poly(xs, degree=3, raw=TRUE)))
regplot(res.cub, mod=2, pred=sav, xvals=xs , xlim=c(4,10),
        ylim=c(-1,1), las=1, digits=1, bty="o", xlab="Total weekly exercise duration", ylab="Hedges'g", cex.lab=1.3)
...
knots
sav <- predict(res.rcs3, newmods=rcspline.eval(xs, knots, inclx=TRUE))
tmp <- regplot(res.rcs3, mod=2, pred=sav, xvals=xs, las=1, digits=1, bty="l", xlab="exercise intensity", ylab="Hedges'g")
abline(v=knots, lty="dotted")
points(tmp)
#在视觉临界点处添加阈值线
abline(v=18, lty="dashed", lwd=2, col="black")
...
res[,c("age", "Per_session_exercise_duration", "Exercise_time_per_week","Exercise_Intensity","int_weeklyvolume","Frequency")] %>% cor()

res1 <- rma.mv(yi = yi,
               V = vi,
               data = res,
               random = ~ 1 | author/studyid,
               method = "REML", # Right away, as this is the final model
               test = "t",
               slab = author,
               mods = ~ age + Per_session_exercise_duration + Exercise_Intensity)
res1