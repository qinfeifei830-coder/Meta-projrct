...
my_data<-read_excel("C:/Users/26069/Desktop/游戏化运动对于青少年/游戏化运动对于青少年基线表格.xlsx", 
                              sheet = "Sex（Male proportion）")

#将强度转换为因子变量
full.model <- rma.mv(G, ViG, slab = author, random = ~ 1 | studyid,  test = "t", method = "REML", data=my_data,mods = ~ Subgroup)
full.model
...
# 步骤 2：使用排序后的数据拟合模型
full.model_sorted <- rma.mv(G, ViG, slab = author, random = ~ 1 | studyid, test = "t", method = "REML", data = my_data_sorted,mods = ~ Subgroup)
...
# 在不考虑潜在异常值的情况下重复该分析
full.model_no_outliers <- rma.mv(yi = G, V = ViG, slab = author, random = ~ 1 | studyid, test = "t", method = "REML", data = df_no_outliers,mods = ~ Subgroup)
...
#线性回归
res.lin <- rma(G, ViG, mods = ~ sex, data=res )
res.lin
regplot(res.lin, las=1, digits=1, bty="l", 
        xlab="sex", ylab="Hedges'g")
res.cub <- rma(G, ViG,, mods = ~ poly(sex, degree=3, raw=TRUE), data=res )
res.cub
xs <- seq(-1, 1, length=5)
...
res.rcs3<- rma(G, ViG,mods = ~ rcs(sex, 3), data=res)
res.rcs3
knots <- attr(rcs(res$sex, 3), "parms")
knots
